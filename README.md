...
<img src="frontend/public/brand/e-bank-logo.svg" alt="E-Bank" height="64" />
...
**Landing / sign in**
<img src="docs/screenshots/01-landing-login.png" alt="Sign-in page" width="100%" />
...
**Admin — reports**
<img src="docs/screenshots/03-admin-reports.png" alt="Admin reports" width="100%" />

</td>
<td width="50%">

**Customer — accounts overview**
<img src="docs/screenshots/04-customer-dashboard.png" alt="Customer dashboard" width="100%" />
...
<img src="docs/screenshots/05-customer-transactions.png" alt="Customer transaction history" width="100%" />

</td>
<td width="50%">

**Kafka, visualized** ([Kafka UI](https://github.com/provectus/kafka-ui))
<img src="docs/screenshots/06-kafka-ui-topics.png" alt="Kafka UI topic list" width="100%" />
...
- **Admin back office** — manage customers and accounts, review every transaction,
  generate PDF/CSV reports with real tables (not JSON dumps).
...
- **Transfer audit trail** — every transfer's two legs (debit + credit) are linked
...
- **Idempotent money commands** — retrying a `credit`/`debit`/`transfer` with the same
  `Idempotency-Key` is always safe, even under heavy concurrent retries.
...
| Testing | JUnit 5, Mockito, Playwright (desktop + mobile), Python verification scripts |
...
|---|---|---|
| **discovery-service** | 8761 | Eureka registry. Every other service registers here; the gateway asks it "who is `account-service` right now?" |
| **gateway-service** | 8080 | Single entry point. Validates the JWT, rate-limits, and routes `/api/**` to the right service via Eureka. |
| **customer-service** | 8081 | Auth (login/register), user & customer records. Publishes `customer-events`. |
| **account-service** | 8082 | Current & savings accounts, balances. Applies money commands idempotently (a `BalanceReceipt` keyed by operation ID). Publishes `account-events` / `account-balance-updates`. |
...
A request for `/api/accounts/{id}` goes **Frontend → Gateway → Eureka (resolve) →
account-service → back**. Nothing is proxied blind — the gateway always asks Eureka
...
sequenceDiagram
    ...
    Note over U,RS: The customer's activity feed and the<br/>admin dashboard both pick this up from Kafka —<br/>not by reading transaction-service's database directly.
```
...
- **Exactly-once projections.** `reporting-service` deduplicates by event ID and
  rejects stale/out-of-order snapshots, so replaying the same event (or restarting
...
message each, then watch reporting-service's dashboard/activity feed update a
...
Zookeeper + Kafka + Kafka UI, and seeds 50 demo customers with 7 months of
...
| `generate-demo-data.py` | regenerates the seed dataset (accounts + operations, pre-reconciled) |
...
├── scripts/                      # Python verification & demo-data scripts
└── docs/                         # screenshots, architecture notes, test evidence
```

Each service's own `src/main/java/.../` follows the same shape: `controllers/` →
`services/` → `repositories/` / `entities/`, with `clients/` for Feign calls to
